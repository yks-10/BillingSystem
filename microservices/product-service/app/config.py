...
class ProductServiceSettings(BaseServiceSettings):
    ...
    SERVICE_NAME: str = Field(default="product-service", description="Service name")
    SERVICE_PORT: int = Field(default=8001, description="Service port")
    
    # Database
    DATABASE_URL: str = Field(
        default="postgresql://postgres:password@localhost:5432/product_db",
        description="Product database URL"
    )
